...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        if [ "$static_mpi" = no ] ; then
          ...
          $Show $FC $PROFILE_INCPATHS ${final_fcflags} ${final_ldflags} "${allargs[@]}" $FCINCDIRS $FCMODDIRS -L$libdir -lmpifort $PROFILE_PRELIB $PROFILE_FOO $rpath_flags $libdir/libmpi.a  $PROFILE_POSTLIB ${final_libs}  ${fabric_dep}
        fi
        rc=$?
    fi
else
