...
int MPI_Comm_split(MPI_Comm comm, int color, int key, MPI_Comm * newcomm)
{
   ...
   if (PRINT_FUN_NAME){
		...
		printf("[Utility rank = %d  color=%d  key=%d] Func: MPI_Comm_split\n", init_rank,color,key);fflush(stdout);
	}
#endif
    init_phase = 1;
    int mpi_errno = MPI_SUCCESS;
    ...
    if (newcomm_ptr){
        ...
        /* Create concurrent_comm for the new communicator */
        if (init_phase==0 && newcomm_ptr->concurrent_comm == NULL){
            mpi_errno = create_concurrent_comm(comm_ptr->handle, comm_ptr->local_size, comm_ptr->rank);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }

        /* Gather new ranks to find number of new distinct communicator groups */
        // int* new_ranks;
        // MPIR_CHKLMEM_DECL(2);
        // MPIR_CHKLMEM_MALLOC(new_ranks, int *, comm_ptr->local_size * sizeof(int),
        //                     mpi_errno, "new_ranks", MPL_MEM_COMM);
        // MPIR_Gather(&(newcomm_ptr->rank), 1, MPI_INT, new_ranks, 1, MPI_INT, 0, comm_ptr, &errflag);
        // /* Send idx and count to each rank */
        // if(comm_ptr->rank ==0){
        //     int li = 0, count=0, idx;
        //     for(; li<comm_ptr->local_size; ++li){
        //         if(new_ranks[li] == 0)
        //             ++count;
        //     }
        //     for(; li<comm_ptr->local_size; ++li){
        //         if(new_ranks[li] == 0){
        //             int group_idx = 0;
        //             // if new rank here is 0?
        //             mpi_errno = MPIC_Send(&count, 1, MPI_INT, new_ranks[li], 100,
        //                                 comm_ptr, &errflag);
        //             if (mpi_errno)
        //                 MPIR_ERR_POP(mpi_errno);
        //             mpi_errno = MPIC_Send(&group_idx, 1, MPI_INT, new_ranks[li], 101,
        //                                 comm_ptr, &errflag);
        //             if (mpi_errno)
        //                 MPIR_ERR_POP(mpi_errno);
        //             ++group_idx;
        ...
        //     }
        // }
        // if(newcomm_ptr->rank==0){
        //     int count, idx;
        //     mpi_errno = MPIC_Send(&count, 1, MPI_INT, 0, 100,
        //                                 comm_ptr, &errflag);
        //     if (mpi_errno)
        //         MPIR_ERR_POP(mpi_errno);
        //     mpi_errno = MPIC_Send(&idx, 1, MPI_INT, 0, 101,
        //                                 comm_ptr, &errflag);
        //     if (mpi_errno)
        //         MPIR_ERR_POP(mpi_errno);

        //     printf("for %d, count = %d, idx = %d\n", comm_ptr->rank, count, idx);
        //     newcomm_ptr->shmem_idx = (int)(comm_ptr->local_size / count) * idx;
        //     newcomm_ptr->ctx_shmem_idx = (int)(comm_ptr->local_size / count) * idx;

        // }

        // MPIR_CHKLMEM_FREEALL();     
    }
    /******************* End of Added by Mehran *********************/
