...
#if 0
 struct threadInfo{
    volatile int operation_type; /* 1 for encryption, 2 for decryption   */
    volatile int mpiLibrary_type;/* 1=send, 2=recv, 3=isend, 4=irecv,    */
                        /* 5=bcast, 6=alltoall, 7=alltoallv,    */
                        /* 8=allgathre, 9=wait, 10=waitall      */
    volatile int flag;           /* set 1 to perfrome operation          */
    volatile int start;          /* begining of the plaintext or cipher  */    
    volatile int len;            /* length of the text                   */
    volatile int enout_pos;      /* position where output will be write  */
    volatile int deout_pos;      /* position where output will be write  */
    volatile int dest;
    volatile int source;
    volatile int tag;
    volatile int root;
    volatile int finish;
    volatile int send;
    volatile unsigned char * sendbuf;
    volatile unsigned char * recvbuf;
    volatile unsigned char * enc_src;
    volatile unsigned char * enc_dest;
   // volatile unsigned char * nonce;
    volatile unsigned char  nonce[16];
    volatile EVP_AEAD_CTX *ctx;
    volatile int segments;
    volatile int seg_counter;
    volatile int send_counter;
    volatile unsigned int n_counter;
    volatile int recv_start[300]; // keep the position of recv segments
};
struct openmp_comm_queue{
  volatile unsigned char * send_loc;
  volatile unsigned char * recv_loc;
  volatile int segment_counter;
  volatile int send_thread_id;
};
#endif
#define MAX_OPENMP_THREAD_NO 32
#if 0
extern volatile struct openmp_comm_queue opm_send_queue[MAX_OPENMP_THREAD_NO][300];
extern volatile struct openmp_comm_queue opm_recv_queue[MAX_OPENMP_THREAD_NO][300];
extern volatile int openmp_send_q_front[MAX_OPENMP_THREAD_NO];
extern volatile int openmp_send_q_rear[MAX_OPENMP_THREAD_NO];
extern volatile int openmp_recv_q_front[MAX_OPENMP_THREAD_NO];
extern volatile int openmp_recv_q_rear[MAX_OPENMP_THREAD_NO];
#endif
