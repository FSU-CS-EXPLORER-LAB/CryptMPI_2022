...
struct MPIR_Comm {
    ...
    *remote_group;              /* The local and remote groups are the
    ...
    int *internode_table;       /* internode_table[i] gives the rank in
                                 ...
                                 * It is of size 'local_size'. */
	// Mohsen                   /* check if Host-file is arranged as cyclic or blocked */
    int is_uniform;
    int is_blocked; 
    int cyclic_hostfil;
    ...
    // end   
    /*** Added by Mehran ***/
    int shmem_idx;              /* index of shared memory buffer for this communicator */
    int ctx_shmem_idx;          /* index of ciphertext shared memory buffer */
    /***********************/ 
