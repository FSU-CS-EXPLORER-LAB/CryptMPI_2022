...
int MPIR_Find_local_and_external(MPIR_Comm * comm, int *local_size_p, int *local_rank_p,
                                 ...
                                 int **internode_table_p, int **node_sizes_p)
{
    ...
    if (node_cnt==comm->local_size) {
        for (i=0; i<node_cnt; i++)
            node_sizes[i]=1;
    } else if (comm->node_roots_comm != NULL) {
        MPIR_Comm *leader_commptr = NULL;
        MPIR_Comm *shmem_commptr = NULL;
        leader_commptr = comm->node_roots_comm;
        shmem_commptr = comm->node_comm;

        if (comm->node_comm == NULL && comm->node_roots_comm != NULL) {
        ...
        } else if (shmem_commptr->local_size != NULL && comm->rank == 0) {
            node_sizes[0] = shmem_commptr->local_size;        
        }
        
        mpi_errno = MPIR_Allgather_intra_ring(MPI_IN_PLACE, 1, MPI_INT, node_sizes, 1, MPI_INT, comm->node_roots_comm, &errflag);

        // if (comm->rank == 0){
        //     printf("CryptMPI: *** This is node_size[] = ");
        //     for (i=0; i<node_cnt; i++)
