...
//int myThreadId[MY_MAX_NO_THREADS+10];
//pthread_t thread_id[MY_MAX_NO_THREADS+10];
...
double  total_comm_plus_enc_time = 0;
double total_comm_plus_enc_time_long_msg = 0;
double total_comm_plus_enc_time_small_msg = 0;

//double bcast_reduce_allreduce_time = 0;
double bcast_total_time     = 0;
double reduce_total_time    = 0;
double allreduce_total_time = 0;
double alltoall_total_time  = 0;
double alltoallv_total_time = 0;
double send_total_time      = 0;
double isend_total_time     = 0;
double recv_total_time      = 0;
double irecv_total_time     = 0;
double wait_total_time      = 0;
double waitall_total_time   = 0;

double inter_less_than_4K = 0;
double inter_less_than_16K = 0;
double inter_less_than_32K = 0;
double inter_less_than_64K = 0;
...
int max_send_thread_used = 0;
int max_wait_thread_used = 0;    
#endif


#if COLL_LOG_DATA

int max_bcast_datasz = 0;
int max_reduce_datasz = 0;
int max_allreduce_datasz = 0;
#endif
int long_msg_flag = 0;
...
void init_counter_mode_keys(){
    ...
    /* Get the number of threads */
     ...
     {
         char *temp = getenv("MV2_OMP_NUM_THREADS");
         if (temp)
             openmp_active_thread_no = atoi(temp);
         else
             openmp_active_thread_no = 1;
     }
     else
 ...
 no_of_max_omp_threads = omp_get_num_procs();  
   ...
   // MPID_Comm_get_ptr( MPI_COMM_WORLD, cryptmpi_comm_ptr );
    ...
    /*int mpi_errno = MPIU_Find_local_and_external(cryptmpi_comm_ptr,
     ...
     cryptmpi_local_process = 0; //num_local;
     int len;
    MPI_Get_processor_name(cryptmpi_process_name, &len );
    for(i=0;i<total_process_number;i++){
         sameNode[i]= -1;
    ...
    for(i=0;i<total_process_number;i++){
        if(strncmp(cryptmpi_process_name, all_p_names+(i*len), len ) == 0 ){
            sameNode[i] = 1;
            cryptmpi_local_process++;
        }
    }
    //printf("cryptmpi_local_process %d\n",cryptmpi_local_process);
#if PRE_COMPUTE_COUNTER_MODE
    ...
    printf("[rank %d %s] \n%c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c\n"
           ...
           "%c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c %c%c%c%c\n"
           "-----------------------------------------------------------------\n\n",
    cryptmpi_own_rank,cryptmpi_process_name,
    all_p_names[0],all_p_names[1],all_p_names[2],all_p_names[3],all_p_names[4],all_p_names[5],all_p_names[6],all_p_names[7],
    all_p_names[8],all_p_names[9],all_p_names[10],all_p_names[11],all_p_names[12],all_p_names[13],all_p_names[14],all_p_names[15],
    all_p_names[16],all_p_names[17],all_p_names[18],all_p_names[19],all_p_names[20],all_p_names[21],all_p_names[22],all_p_names[23],
    all_p_names[24],all_p_names[25],all_p_names[26],all_p_names[27],all_p_names[28],all_p_names[29],all_p_names[30],all_p_names[31],

    all_p_names[32],all_p_names[33],all_p_names[34],all_p_names[35],all_p_names[36],all_p_names[37],all_p_names[38],all_p_names[39],
    all_p_names[40],all_p_names[41],all_p_names[42],all_p_names[43],all_p_names[44],all_p_names[45],all_p_names[46],all_p_names[47],
    all_p_names[48],all_p_names[49],all_p_names[50],all_p_names[51],all_p_names[52],all_p_names[53],all_p_names[54],all_p_names[55],
    all_p_names[56],all_p_names[57],all_p_names[58],all_p_names[59],all_p_names[60],all_p_names[61],all_p_names[62],all_p_names[63],

    all_p_names[64],all_p_names[65],all_p_names[66],all_p_names[67],all_p_names[68],all_p_names[69],all_p_names[70],all_p_names[71],
    all_p_names[72],all_p_names[73],all_p_names[74],all_p_names[75],all_p_names[76],all_p_names[77],all_p_names[78],all_p_names[79],
    all_p_names[80],all_p_names[81],all_p_names[82],all_p_names[83],all_p_names[84],all_p_names[85],all_p_names[86],all_p_names[87],
    all_p_names[88],all_p_names[89],all_p_names[90],all_p_names[91],all_p_names[92],all_p_names[93],all_p_names[94],all_p_names[95],

    all_p_names[96],all_p_names[97],all_p_names[98],all_p_names[99],all_p_names[100],all_p_names[101],all_p_names[102],all_p_names[103],
    all_p_names[104],all_p_names[105],all_p_names[106],all_p_names[107],all_p_names[108],all_p_names[109],all_p_names[110],all_p_names[111],
    all_p_names[112],all_p_names[113],all_p_names[114],all_p_names[115],all_p_names[116],all_p_names[117],all_p_names[118],all_p_names[119],
    all_p_names[120],all_p_names[121],all_p_names[122],all_p_names[123],all_p_names[124],all_p_names[125],all_p_names[126],all_p_names[127]); fflush(stdout);

    printf("[rank %d %s] \n%d%d%d%d%d%d%d%d %d%d%d%d%d%d%d%d %d%d%d%d%d%d%d%d %d%d%d%d%d%d%d%d\n"
           "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-\n\n",
    cryptmpi_own_rank,cryptmpi_process_name,
    sameNode[0],sameNode[1],sameNode[2],sameNode[3],sameNode[4],sameNode[5],sameNode[6],sameNode[7],
     sameNode[8],sameNode[9],sameNode[10],sameNode[11],sameNode[12],sameNode[13],sameNode[14],sameNode[15],
      sameNode[16],sameNode[17],sameNode[18],sameNode[19],sameNode[20],sameNode[21],sameNode[22],sameNode[23],
       sameNode[24],sameNode[25],sameNode[26],sameNode[27],sameNode[28],sameNode[29],sameNode[30],sameNode[31]); fflush(stdout);
#endif       
    ...
    */  
    ...
    if (comm_ptr->cyclic_hostfil==1){
        for (i = 0; i < comm_ptr->remote_size; i++){
            comm_rank_list[i] = (i / ppn) + ((i % node_cnt) * ppn);
