...
int MPI_SEC_MultiThread_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request)
{
    ...
    *request = nonBlockCounter;
    ...
    //printf("   >> irecv call [rank=%d] recv from source=%d tag=%d\n",comm_ptr->rank,source,tag);fflush(stdout);

    /* Post all pipe line request */
    /* Received Header bytes  for data size with original tag */
    //mpi_errno = MPI_Irecv_original(&Ideciphertext[nonBlockCounter][0], (recvtype_sz * count) + MSG_HEADER_SIZE , MPI_UNSIGNED_CHAR, source, tag, comm,  &nonblock_req_handler[nonBlockCounter].request[0]);
  ...
  // printf("irecv MY_OPENMP_THREADS_NO =%d\n",MY_OPENMP_THREADS_NO);fflush(stdout);
  int my_thread_no = MY_OPENMP_THREADS_NO;
    ...
    if(data == 0)
        data = 1;
     segments_no = (totaldata -1)/data + 1;
     if(segments_no < 1)
      segments_no = 1;
    if(OPENMP_THREADS)
        ...
        mpi_errno = MPI_Irecv_original(&Ideciphertext[nonBlockCounter][th_start], totaldata+(16+12)*MY_MAX_NO_THREADS, MPI_CHAR, source,
                                                tag, comm, &nonblock_req_handler[nonBlockCounter].request[req_counter++]);
...
/*
- use a different tag to solve mismatched message size
- probe and iprobe
*/
...
int MPI_SEC_Irecv2_largeSegment_3(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request)
{
  ...
  //    printf(COLOR_BLUE"    %d recv from %d totaldata = %d tag =%d"COLOR_RESET"\n", comm_ptr->rank, source, totaldata,tag );fflush(stdout);
...
#if 0 //OMP_DYNAMIC_THREADS_PIPELINE
    if (totaldata < SIXTY_4K)
        my_thread_no = 1;
    else if (totaldata < ONE_TWO_8K)
        my_thread_no = 2;
        ...
        my_thread_no = 8;
#elif 0 //OMP_DYNAMIC_THREADS_PIPELINE_INNOVATION
    if (totaldata < SIXTY_4K)
        my_thread_no = 1;
   ...
   // newtag[source] = newtag[source] + 1;
  //  tag = recvtag[source] + 1;
    // printf(COLOR_BLUE"    %d recv from %d totaldata = %d tag =%d"COLOR_RESET"\n", comm_ptr->rank, source, totaldata,tag );fflush(stdout);
   ...
   // tag++;
    ...
    choping_sz = PIPELINE_SIZE; //LARGE_SEGMENT_SIZE; // SIXTY_4K; //THIRTY_2K; // totaldata+200;; // consider minimum and post twice of it.
  ...
  /*  int temp_thread;
    ...
    inner_segments = (PIPELINE_SIZE - 1) / my_thread_no + 1;
    inner_segments = (PIPELINE_SIZE - 1) / inner_segments + 1;*/
    
    inner_segments = MAX_OMP_THREADS_LIMIT;

#elif OMP_DYNAMIC_THREADS_PIPELINE_INNOVATION
    choping_sz = 16384; // totaldata+200;; // consider minimum and post twice of it.
    max_data_recv_size = totaldata + MAX_OMP_THREADS_LIMIT * (NONCE_HEADER + ENC_MSG_TAG_SIZE);
#endif
