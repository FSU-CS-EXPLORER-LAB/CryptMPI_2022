                          ...
                          MPI_Comm comm, MPI_Request *reqst)
{
    ...
    EVP_AEAD_CTX *segment_ctx[20]; // total threads will not excede 100
...
#pragma omp parallel default(none) private(segment_counter, nonce_counter, local_nonce, ciphertext_len, mpi_errno, i, base,max_out_len) shared(buf, Iciphertext, th_data, segment_ctx, segments_no, local_ctx, my_thread_no, nonblock_req_handler, nonBlockCounter,totaldata) num_threads(my_thread_no)
  {
...
#pragma omp parallel default(none) private( nonce_counter, local_nonce, ciphertext_len, mpi_errno, base) shared(buf, Iciphertext, th_data, segment_ctx, segments_no, dest, tag, comm, local_ctx, my_thread_no, nonblock_req_handler, nonBlockCounter, request_counter, pendingIsendRequestCount, inner_totaldata,segment_counter,ii,prsd_segment,enc_loc,totaldata,send_loc) num_threads(my_thread_no)
{
...
#pragma omp parallel for default(none) private( nonce_counter, local_nonce, ciphertext_len, mpi_errno, base) shared(buf, Iciphertext, th_data, segment_ctx, segments_no, dest, tag, comm, local_ctx, stdout, my_thread_no, nonblock_req_handler, nonBlockCounter, request_counter, pendingIsendRequestCount, inner_totaldata,segment_counter,ii,prsd_segment,enc_loc,totaldata,send_loc) num_threads(my_thread_no)
             ...
             for (inner_segment_counter = 0; inner_segment_counter < ii; inner_segment_counter++)
            {
               ...
               // base = MSG_HEADER_SIZE + nonce_counter * (th_data + ENC_MSG_TAG_SIZE + NONCE_HEADER) + NONCE_HEADER;
	     ...
	     MPI_Comm comm,  MPI_Request *request)
{
    ...
    // printf("isend: omp_get_max_threads() %d\n",omp_get_max_threads());fflush(stdout);  
    MPIR_Comm *comm_ptr = NULL;
    MPIR_Comm_get_ptr(comm, comm_ptr);         
    ...
    /* setting the size of the data */
     ...
     Iciphertext[nonBlockCounter][3] = tempdata & 0xFF;
      ...
      /* Mark this is a Blocking Send from iSend */
    ...
    int my_thread_no =  1; //MY_OPENMP_THREADS_NO; //  omp_get_max_threads();
#if OMP_DYNAMIC_THREADS
    if(totaldata < SIXTY_4K)
        my_thread_no = 1;
    else if( totaldata < ONE_TWO_8K)
   ...
   if(my_thread_no > cyptmpi_series_thread)
    ...
    int nodeid1, nodeid2, nodeid3, nodeid4;
     MPID_Get_node_id(comm_ptr, comm_ptr->rank, &my_nodeid);
     MPID_Get_node_id(comm_ptr, 1, &nodeid1);
     MPID_Get_node_id(comm_ptr, 2, &nodeid2);
     MPID_Get_node_id(comm_ptr, 3, &nodeid3);
     MPID_Get_node_id(comm_ptr, 4, &nodeid4);
    printf("myhostname =%s rank = %d cryptmpi_local_process=%d no_of_max_omp_threads=%d my_nodeid=%d cryptmpi_local_rank=%d nodeid1=%d nodeid2=%d nodeid3=%d nodeid4=%d\n",
    myhostname, comm_ptr->rank, cryptmpi_local_process,no_of_max_omp_threads,my_nodeid,cryptmpi_local_rank, nodeid1,nodeid2,nodeid3,nodeid4);fflush(stdout);
    */
    ...
    if(!(cryptmpi_local_process*12 < no_of_max_omp_threads)){
        ...
        *(cryptmpi_ranks+cryptmpi_local_rank) = my_thread_no;
        ...
        printf("    isend rank =%d  thread_sum=%d cryptmpi_local_process=%d diff = %d no_of_max_omp_threads=%d my_thread_no=%d\n",comm_ptr->rank,thread_sum,cryptmpi_local_process,diff, no_of_max_omp_threads, my_thread_no);
        fflush(stdout);
#endif        
    }
#endif                                                
  ...
  //  #pragma omp parallel for  schedule(static) default(none) private(segment_counter,count,max_out_len,pos,ciphertext_len) shared(buf, Iciphertext,segment_no,data,stdout,global_openmp_ctx,totaldata,nonBlockCounter)  num_threads(my_thread_no)  
   #pragma omp parallel for  schedule(static) default(none) private(segment_counter,count,max_out_len,pos,ciphertext_len) shared(buf, Iciphertext,segment_no,data,global_openmp_ctx,totaldata,nonBlockCounter)  num_threads(my_thread_no)  
#endif    
...
/*
* v5 : same as v4 for isend
*
*/
int MPI_SEC_PreComputeCounter_Isend_v5(const void *buf, int count, MPI_Datatype datatype, int dest, int tag,
              MPI_Comm comm, MPI_Request *request)
{
    ...
    // Iciphertext[nonBlockCounter][13] = '1';
...
/* Mark this is a Blocking Send from Send */
    ...
    */
    ...
    if (totaldata < PRE_COM_DATA_RANGE)
    {
        ...
        if(percentage >= 100)
            {printf(COLOR_GREEN"[isend enc rank %d] percentage %lf common_compute_size %d totaldata %d"COLOR_RESET"\n",comm_ptr->rank,percentage,common_compute_size,totaldata);fflush(stdout);}
        else if(percentage >= 75)
            {printf(COLOR_BLUE"[isend enc rank %d] percentage %lf common_compute_size %d totaldata %d"COLOR_RESET"\n",comm_ptr->rank, percentage, common_compute_size,totaldata);fflush(stdout);}
        else if(percentage >= 50)
            {printf(COLOR_CYAN"[isend enc rank %d] percentage %lf common_compute_size %d totaldata %d"COLOR_RESET"\n",comm_ptr->rank, percentage, common_compute_size,totaldata);fflush(stdout);} 
         else if(percentage >= 25)
            {printf(COLOR_MAGENTA"[isend enc rank %d] percentage %lf common_compute_size %d totaldata %d"COLOR_RESET"\n",comm_ptr->rank, percentage, common_compute_size,totaldata);fflush(stdout);}
        else
            {printf(COLOR_YELLOW"[isend enc rank %d] percentage %lf common_compute_size %d totaldata %d"COLOR_RESET"\n",comm_ptr->rank, percentage, common_compute_size,totaldata);fflush(stdout);}                                              
        */
       /* if(common_compute_size < totaldata)
        {
...
#pragma omp parallel default(none) private(inner_segment_counter, base, enc_data, enc_buffer, in_buffer, out_buffer, j, t_mask, t_mask_pos, t_counter, t_counter_data) shared(buf, Iciphertext, enc_common_buffer, local_counter_ctx, th_data, segments_no, my_thread_no, inner_totaldata, segment_counter, ii, totaldata, send_loc, enc_common_end, rank, enc_common_start, common_compute_size, enc_common_counter_long_msg,nonBlockCounter) num_threads(my_thread_no)
  {
...
{ 
#if CRYPTMPI_COUNTER_MODE       
        mpi_errno = MPI_SEC_PreComputeCounter_Isend_v5(buf, count, datatype, dest, tag, comm, request);
#endif
}
    else
    {
