...
//#if OPENMP_THREADS
//#pragma omp parallel for schedule(static) default(none) private(segment_counter, count, max_out_len, pos) shared(buf, large_recv_buffer, segment_no, data, global_openmp_ctx, totaldata) num_threads(my_thread_no)
//#endif
...
//#endif
    ...
    if (totaldata <= PIPELINE_SIZE && (large_recv_buffer[20] != '3' && large_recv_buffer[20] != '4'))
    {
        ...
        local_nonce[7] = large_recv_buffer[0];
        local_nonce[8] = large_recv_buffer[1];
        local_nonce[9] = large_recv_buffer[2];
        local_nonce[10] = large_recv_buffer[3];
        local_nonce[11] = large_recv_buffer[4];*/

        //nonce_counter = ((unsigned char)large_recv_buffer[4] << 0) | ((unsigned char)large_recv_buffer[3] << 8) | ((unsigned char)large_recv_buffer[2] << 16) | ((unsigned char)large_recv_buffer[1] << 24);
        // tinfo[m].deout_pos = nonce_counter * th_data;

        // printf("wait for 1st decrypt recv_len=%d\n",recv_len);fflush(stdout);
...
/* Switch Between CryptMPI-R and CryptMPI-C */
    ...
    if( omp_get_team_size(1) > -1) // if true then called from parallel region switch to CryptMPI-C
        ...
        else
        {
            ...
            //printf("inside_segments = %d\n",inside_segments);fflush(stdout);
...
#pragma omp parallel default(none) private(nonce_counter, local_nonce, decrypted_len, mpi_errno, m, pos, dec_pos, segment_counter) shared(buf, large_recv_buffer, th_data, segment_ctx, new_segments, local_ctx, totaldata) num_threads(my_thread_no)
  {  
    ...
    if( omp_get_team_size(1) > -1) // if true then called from parallel region switch to CryptMPI-C
        my_thread_no = 1;
            ...
            for (segment_counter = 0; segment_counter < segments_no; segment_counter++)
            {
                ...
                //if(active_thread_num+segment_counter < segments_no){
                //  for(m=0;m<active_thread_num;m++)
...
#pragma omp parallel default(none) private(nonce_counter, local_nonce, decrypted_len, mpi_errno, m, pos, dec_pos) shared(buf, large_recv_buffer, th_data, segment_ctx, segments_no, local_ctx, ii, segment_counter, totaldata, enc_loc, inner_totaldata, recv_loc) num_threads(my_thread_no)
{
...
/*
* v5
* if sender use precomputation encrypt directly
* if sender use counter encryption then precompute the whole message
*
*
*/
int MPI_SEC_PreComputeCounter_Recv_v5(void *buf, int count, MPI_Datatype datatype, int source, int tag,
                                      MPI_Comm comm, MPI_Status *status)
{
    int mpi_errno = MPI_SUCCESS;
    unsigned long next, decrypted_len, dest;
    ...
    if (totaldata < PRE_COM_DATA_RANGE)
    {
        ...
        common_recv_counter += (unsigned long)(totaldata - 1) / 16 + 1;

       
        mpi_errno = MPI_Wait_original(&request[0], &sta);
        //printf("totaldata %d common_recv_counter %lu %2x %2x \n",totaldata, common_recv_counter,dec_common_buffer[0],large_recv_buffer[0]);fflush(stdout);
        decryption_common_counter(buf, totaldata, totaldata, 0, 0, 0, common_recv_counter, source, large_recv_buffer);
        // EVP_EncryptUpdate(global_counter_ctx, buf, &len, large_recv_buffer, totaldata); // it has bad performance than precomputation
#endif
    ...
    else
    {
       ...
       /* if (totaldata % PIPELINE_SIZE == 0 && totaldata >= PIPELINE_SIZE)
            common_counter_gen_sz *= segments_no;
        else if (totaldata >= PIPELINE_SIZE)
            common_counter_gen_sz *= (segments_no - 1);*/

        /*
        memcpy(iv_buffer, &Recv_common_IV[source * 16], 16);
        IV_Count(iv_buffer, common_recv_counter);
        EVP_EncryptInit_ex(global_counter_ctx, NULL, NULL, NULL, iv_buffer);
        EVP_EncryptUpdate(global_counter_ctx, dec_common_buffer, &len, zeros, common_counter_gen_sz);
       ...
       // int m_t_no, m_t_data, m_t_segments, t_end;
        /* Determine the thread number for pre-computation */

       /* if(totaldata >= FIVE_ONE_2K)
            gen_size = FIVE_ONE_2K;
        else
        {
            gen_size = totaldata;
        }
        if (gen_size < THIRTY_2K)
            m_t_no = 1;
        else if (gen_size < SIXTY_4K)
            m_t_no = 4; 
        else if (gen_size <= TWO_FIVE_6K)
            m_t_no = 8; //8;
        else if (gen_size > TWO_FIVE_6K)
            m_t_no = 16;

         if (m_t_no > cyptmpi_series_thread)
            m_t_no = cyptmpi_series_thread;*/    

        /* Each thread will generate th_data amount mask, th_data%16 == 0*/
        
        // m_t_no = 24;
        //m_t_data = FOUR_K ;
       /* m_t_data = gen_size / m_t_no;
        m_t_data = ((m_t_data - 1) / 16) * 16 + 16;  
        m_t_segments = m_t_no;   */
       ...
       // long unsigned int temp_common_recv_counter = common_recv_counter;
        for (segment_counter = 0; segment_counter < segments_no; segment_counter++)
        {
            ...
            /* use multi-thread to generate mask */
              
#if 0            
            flag = 0;
            MPI_Test(&request[segment_counter], &flag, &sta);
            while (!flag)
            {
                if(((dec_cmn_end + m_t_data*m_t_segments) <= MAX_COMMON_COUNTER_SZ) && ((so_far_done +  m_t_data*m_t_segments) <= temp_totaldata)) 
                {
#pragma omp parallel for default(none) schedule(static) private( j, t_end, t_counter,iv_buffer, len) shared( dec_common_buffer, local_counter_ctx, m_t_segments, stdout, source, common_recv_counter, common_counter_gen_sz, m_t_data, dec_cmn_end, Recv_common_IV, zeros) num_threads(m_t_no)                    
                    for(j=0;j<m_t_segments;j++)
                    {
                        t_end = dec_cmn_end + j * m_t_data;
                        if(j> 0)
                            t_counter = common_recv_counter +  (unsigned long)(j * m_t_data - 1) / 16 + 1;
                        else
                            t_counter = common_recv_counter; 
#if DEBUG_PRECOMPUTE_COUNTER
                    int tid = omp_get_thread_num();              
                    printf(COLOR_MAGENTA "[recv dec mask: tid = %d]: j %d  t_end=%d m_t_data = %d t_counter %lu common_counter_gen_sz =%d m_t_segments=%d" COLOR_RESET "\n",
                        tid, j, t_end, m_t_data, t_counter, common_counter_gen_sz, m_t_segments);
                    fflush(stdout);
#endif
                        memcpy(&iv_buffer[j * 16], &Recv_common_IV[source * 16], 16);
                        IV_Count(&iv_buffer[j * 16], t_counter);
                        EVP_EncryptInit_ex(local_counter_ctx[j], NULL, NULL, NULL, &iv_buffer[j * 16]);
                        EVP_EncryptUpdate(local_counter_ctx[j], (dec_common_buffer+t_end), &len, zeros, m_t_data);
                    }
                    
                    
                    common_recv_counter += (unsigned long)((m_t_data*m_t_segments) - 1) / 16 + 1;
                    dec_cmn_end += (m_t_data*m_t_segments);
                    so_far_done += (m_t_data*m_t_segments);
                    common_counter_gen_sz += (m_t_data*m_t_segments);
                    //printf("rank %d dec_cmn_end %d gen_size %d common_counter_gen_sz %d common_recv_counter %lu\n",
                    //thisrank, dec_cmn_end, gen_size,common_counter_gen_sz, common_recv_counter);
                    MPI_Test(&request[segment_counter], &flag, &sta);
                }
                else
                {
...
#pragma omp parallel default(none) private( m, pos, dec_pos, enc_data, t_mask, t_mask_pos, t_counter,t_counter_data,iv_buffer,len) shared(buf, large_recv_buffer,dec_common_buffer, th_data, local_counter_ctx, segments_no, ii, segment_counter, totaldata, dec_loc, inner_totaldata, recv_pos, source, common_recv_counter, common_counter_gen_sz, mask_pos,Recv_common_IV) num_threads(my_thread_no)
    {
    ...
    /* sanity check */
    /*
    char * sanity = (char *)malloc(totaldata * sizeof(char));
    memset(sanity,'a',totaldata);
    MPIR_Comm *comm_ptr_debug = NULL;
    MPIR_Comm_get_ptr(comm, comm_ptr_debug);
    int thisrank = comm_ptr_debug->rank;
    if (strncmp(sanity, buf, totaldata) == 0)
    {
        printf("rank = %d precompute recv: totaldata =%d choping_sz =%d common_recv_counter =%d\n",thisrank,totaldata, choping_sz,common_recv_counter);fflush(stdout);
        
    }
    else
    {
         printf(COLOR_RED"rank = %d precompute recv: totaldata =%d choping_sz =%d common_recv_counter =%d"COLOR_RESET"\n",thisrank,totaldata, choping_sz,common_recv_counter);fflush(stdout);
    }
    free(sanity);
    */
    return mpi_errno;
...
{ 
#if CRYPTMPI_COUNTER_MODE    
        mpi_errno = MPI_SEC_PreComputeCounter_Recv_v5(buf, count, datatype, source, tag, comm, status);
#endif
}   
    else
    {
