...
#pragma omp parallel default(none) private(m, pos, dec_pos, enc_data, t_counter, t_counter_data, iv_buffer, len) shared(buf, Ideciphertext, th_data, local_counter_ctx, segments_no, stdout, ii, segment_counter, totaldata, dec_loc, inner_totaldata, recv_pos, source, recv_counter, index, Recv_common_IV) num_threads(my_thread_no)
                {
