...
//#if OPENMP_PIPE_LINE
    ...
    {
        Crypthandle_probe[source].no_tag += 1;
        int ind = Crypthandle_probe[source].no_tag;
        Crypthandle_probe[source].tags[ind] = tag;
        MPI_Status sta;
        if (flag)
        {
            mpi_errno = MPI_Recv_original(&Crypthandle_probe[source].header[ind][0], MSG_HEADER_SIZE, MPI_UNSIGNED_CHAR, source, tag, MPI_COMM_WORLD, &sta);
            unsigned int count = ((unsigned char)Crypthandle_probe[source].header[ind][3] << 0) | ((unsigned char)Crypthandle_probe[source].header[ind][2] << 8) | ((unsigned char)Crypthandle_probe[source].header[ind][1] << 16) | ((unsigned char)Crypthandle_probe[source].header[ind][0] << 24);
            if (count < 0)
                count = 0;
