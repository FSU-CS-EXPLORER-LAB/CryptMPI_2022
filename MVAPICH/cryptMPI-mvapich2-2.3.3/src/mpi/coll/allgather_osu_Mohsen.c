    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        char hostname[100];
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_Direct_MV2  count = NA\n",comm_ptr->rank,hostname);fflush(stdout);}
#endif  
    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_DirectSpread_MV2  count = NA\n",comm_ptr->rank,hostname);fflush(stdout);}
#endif      
                          ...
                          MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_RD_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif     MPIR_TIMER_START(coll,allgather,rd);
    int comm_size, rank;
    int mpi_errno = MPI_SUCCESS;
    int mpi_errno_ret = MPI_SUCCESS;
    MPI_Aint recvtype_extent;
...
/* Mohsen */								
 if (ALLGATHER_PRINT_FUN_NAME)     fprintf(stderr,"[r = %d]: MPIR_Allgather_RD_MV2 [3] count = %d\n",comm_ptr->rank,sendcount);								
				/*if (rank == 0){
				    printf("Encrypted %d\n", enc_idx);
				}*/
                          ...
                          MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_Encrypted_RDB_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif  
                          ...
                          MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_NaivePlus_RDB_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif  
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_Bruck_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif      
        ...
        if(security_approach==2005){
            ...
            if(node_id != dst_node_id){
                ...
                for(; last_encrypted_index<curr_cnt; ++last_encrypted_index){
					...
					if (ALLGATHER_PRINT_FUN_NAME)     fprintf(stderr,"[r = %d]: MPIR_Allgather_Bruck_MV2 [3] count = %d\n",comm_ptr->rank,sendcount);
                            ...
                            MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_Ring_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif  
    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_gather_bcast_MV2  count = %d\n",comm_ptr->rank,hostname,sendcount);fflush(stdout);}
#endif      
                             ...
                             MPI_Datatype recvtype, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_intra_MV2  count = %d\n",comm_ptr->rank,hostname,sendcount);fflush(stdout);}
#endif      
    int comm_size;
    int mpi_errno = MPI_SUCCESS;
    int type_size;
                            ...
                            MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_2lvl_SharedMem_Allgather_MV2 SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcnt);fflush(stdout);}
#endif      
    int rank, size;
    ...
    if (local_rank == 0 && (leader_comm_size > 1)) {
        ...
        //End of NAIVE PLUS
            ...
            mpi_errno = MPIR_Allgather_impl((void*)((char*)shmem_buffer + (my_node*p * recvcnt * recvtype_extent)), 
                                                (recvcnt*p),
                                                recvtype,
...
int MPIR_2lvl_SharedMem_Concurrent_Encryption_Allgather_MV2(const void *sendbuf,int sendcnt, MPI_Datatype sendtype,
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_2lvl_SharedMem_Concurrent_Encryption_Allgather_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcnt);fflush(stdout);}
#endif                                  
    int rank, size;
    int local_rank, local_size, idx;
    int leader_comm_size = 0, leader_rank; 
                            ...
                            MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_2lvl_Allgather_Encrypted_RDB_MV2 SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcnt);fflush(stdout);}
#endif      
    int rank, size;
    int local_rank, local_size;
    int leader_comm_size = 0, leader_rank; 
                            ...
                            MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_2lvl_Allgather_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcnt);fflush(stdout);}
#endif      
    int rank, size;
    int local_rank, local_size;
    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_2lvl_Allgather_nonblocked_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcnt);fflush(stdout);}
#endif      
    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_2lvl_Allgather_Multileader_RD_nonblocked_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif  
    ...
    if(security_approach==2007 && overlap_decryption==0){
        for (i=0; i < n; ++i){
            if(i != my_node){
    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_Direct_MV2  SEC_AP=%d  OVLAP_DEC=%d count = %d\n",comm_ptr->rank,hostname,security_approach,overlap_decryption,sendcount);fflush(stdout);}
#endif      
    ...
    MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    /***************** Added by Mehran *****************/
    int send_idx=0;
    if(security_approach==2005 && local_rank==local_size-1){
    ...
    MPID_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (ALLGATHER_PRINT_FUN_NAME || DEBUG_INIT_FILE) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allgather rank = %d host = %s] Func: MPIR_Allgather_Direct_MV2  count = %d\n",comm_ptr->rank,hostname,sendcnt);fflush(stdout);}
#endif      
