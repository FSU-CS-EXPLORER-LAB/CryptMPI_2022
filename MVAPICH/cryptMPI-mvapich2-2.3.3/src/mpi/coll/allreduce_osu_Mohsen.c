                             ...
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    if (security_approach==2005)
        ...
        return MPIR_Allreduce_pt2pt_rd_MV2_AESGCM(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                             ...
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
	...
	if (DEBUG_INIT_FILE)  fprintf(stderr,"[1 RD] count=%d rank = %d \n", count,comm_ptr->rank);
...
if (DEBUG_INIT_FILE)  fprintf(stderr,"[2 RD] count=%d rank = %d \n", count,comm_ptr->rank);
    ...
    //fprintf(stderr,"[1] rank = %d  [RD]\n",comm_ptr->rank);
if (DEBUG_INIT_FILE)  fprintf(stderr,"[3 RD] count=%d rank = %d \n", count,comm_ptr->rank);
...
if (DEBUG_INIT_FILE)  fprintf(stderr,"[4 RD] count=%d rank = %d \n", count,comm_ptr->rank);
    // printf("rank = %d rem = %d pof2 = %d comm_size = %d\n",rank, rem, pof2, comm_size); fflush(stdout);
	...
	//if (PRINT_FUN_NAME) printf("max_eager_rd = %d  [Rank = %d]\n",max_eager,comm_ptr->rank);   
...
if (DEBUG_INIT_FILE)  fprintf(stderr,"[5 RD] count=%d rank = %d \n", count,comm_ptr->rank);	
	if (newrank != -1) {
        mask = 0x1;
        while (mask < pof2) {
            newdst = newrank ^ mask;
...
if (DEBUG_INIT_FILE)  fprintf(stderr,"[5 RD] count=%d rank = %d \n", count,comm_ptr->rank);           
			    mpi_errno = MPIC_Sendrecv(recvbuf, count, datatype,
											 ...
											 MPIR_ALLREDUCE_TAG, comm_ptr,
											 MPI_STATUS_IGNORE, errflag);
if (DEBUG_INIT_FILE)  fprintf(stderr,"[6 RD] count=%d rank = %d \n", count,comm_ptr->rank);
			...
			if (DEBUG_INIT_FILE)  fprintf(stderr,"[7 RD] count=%d rank = %d \n", count,comm_ptr->rank);
...
#endif
if (DEBUG_INIT_FILE)  fprintf(stderr,"[8 RD] count=%d rank = %d \n", count,comm_ptr->rank);
				//if (count==256) fprintf(stderr,"BIG\n");
...
#endif                  
				if (DEBUG_INIT_FILE)  fprintf(stderr,"[9 RD] count=%d rank = %d \n", count,comm_ptr->rank);
...
if (DEBUG_INIT_FILE)  fprintf(stderr,"[10 RD] count=%d rank = %d \n", count,comm_ptr->rank);
...
#endif
if (DEBUG_INIT_FILE)  fprintf(stderr,"[11 RD] count=%d rank = %d \n", count,comm_ptr->rank);
        ...
        //printf("done\n");fflush(stdout);
if (DEBUG_INIT_FILE)  fprintf(stderr,"[12 RD] count=%d rank = %d \n", count,comm_ptr->rank);
	...
	/* if (count==64) {
		printf("recvbuf[%d] = %f  %f  %f  rank=%d [RD]\n",count, *((float *) (recvbuf+((10)*extent))), *((float *) (recvbuf+((20)*extent))), *((float *) (recvbuf+((30)*extent))),rank);
		//printf("tmp_buf[%d] = %f  %f  %f  rank=%d [RD]\n",count, *((float *) (tmp_buf+((1)*extent))), *((float *) (tmp_buf+((count/2)*extent))), *((float *) (tmp_buf+((count-1)*extent))),rank);
		fflush(stdout);
	} */
...
if (DEBUG_INIT_FILE)  fprintf(stderr,"[13 RD] count=%d rank = %d \n", count,comm_ptr->rank);
    if (MPIU_THREADPRIV_FIELD(op_errno)) {
        mpi_errno = MPIU_THREADPRIV_FIELD(op_errno);
    }
if (DEBUG_INIT_FILE)  fprintf(stderr,"[14 RD] count=%d rank = %d \n", count,comm_ptr->rank);
  fn_exit:
    MPIU_CHKLMEM_FREEALL();
    MPIR_TIMER_END(coll,allreduce,shm_rd);
if (DEBUG_INIT_FILE)  fprintf(stderr,"[15 RD] count=%d rank = %d \n", count,comm_ptr->rank);	
    return (mpi_errno);
...
#undef FCNAME
#define FCNAME "MPIR_Allreduce_pt2pt_rd_MV2_AESGCM"
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)


int MPIR_Allreduce_pt2pt_rd_MV2_AESGCM(const void *sendbuf,
                             void *recvbuf,
                             int count,
                             MPI_Datatype datatype,
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    int namelen;
    gethostname(hostname, &namelen);
	 printf("[allreduce rank = %d host = %s] Func: MPIR_Allreduce_pt2pt_rd_MV2_AESGCM  count = %d  [COLL_NAIVE_AESGCM]\n",comm_ptr->rank,hostname,count);fflush(stdout);}
#endif     
    MPIR_TIMER_START(coll,allreduce,shm_rd);
    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_shm_rd, 1);
    int comm_size, rank;
                             ...
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE) { char hostname[100];
	...
	if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[1 RSA] count=%d rank = %d \n", count,comm_ptr->rank);
...
if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[1] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
    /* find nearest power-of-two less than or equal to comm_size */
    pof2 = comm_ptr->dev.ch.gpof2;
    ...
    /* In the non-power-of-two case, all even-numbered
       ...
       participate in the algorithm until the very end. The
       remaining processes form a nice power-of-two. */
if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[2] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
    if (rank < 2 * rem) {
        ...
        if (rank % 2 == 0) {
			...
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-1S] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

            MPID_Get_node_id(comm_ptr, MPIR_Rank_list_mapper(comm_ptr, rank + 1), &remote_node_id);
			
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2S] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

            if ((my_node_id == remote_node_id)){
				
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-1S] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
					
                    mpi_errno = MPIC_Send(recvbuf, count, datatype, MPIR_Rank_list_mapper(comm_ptr, rank + 1),
                                     MPIR_ALLREDUCE_TAG, comm_ptr, errflag);
									 
									 if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-2S] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
            } else {
				
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-1*S] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

                    mpi_errno = MPIC_ENC_NAIVE_Send(recvbuf, count, datatype, MPIR_Rank_list_mapper(comm_ptr, rank + 1),
                                     MPIR_ALLREDUCE_TAG, comm_ptr, errflag);
									 
									 if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-2*S] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

// fprintf(stderr,"[*] rank = %d  [RSA-SND Bing]\n",comm_ptr->rank);                                                                       
            }
            ...
            if (!EVP_AEAD_CTX_seal(global_coll_msg_ctx, (large_send_buffer + MSG_HEADER_SIZE),
                ...
                {
                    if (ENABLE_SECURE_DEBUG) printf(COLOR_GREEN"allreduce_rs_mv2: ciphertext_sendbuf_len = %d Rank = %d "COLOR_RESET"\n", ciphertext_sendbuf_len, comm_ptr->rank);
                    fflush(stdout);
                }
#endif   
			...
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-1R] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

            MPID_Get_node_id(comm_ptr, MPIR_Rank_list_mapper(comm_ptr, rank - 1), &remote_node_id);
			
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2R] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

            if ((my_node_id == remote_node_id)){
				
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-1R] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);

                mpi_errno = MPIC_Recv(tmp_buf, count,
                                  datatype, MPIR_Rank_list_mapper(comm_ptr, rank - 1),
                                  MPIR_ALLREDUCE_TAG, comm_ptr,
                                  MPI_STATUS_IGNORE, errflag);
								  if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-2R] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
            } else {
				
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-1*R] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
                ...
                mpi_errno = MPIC_ENC_NAIVE_Recv(tmp_buf, count,
								  ...
								  if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[5-2-2*R] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
 // fprintf(stderr,"[*] rank = %d  [RSA-RCV Bing]\n",comm_ptr->rank);                                  
            }
           ...
           /* mpi_errno = MPIC_Recv(tmp_buf, count, datatype, MPIR_Rank_list_mapper(comm_ptr, rank - 1),
            ...
            {
                if (ENABLE_SECURE_DEBUG) printf(COLOR_BLUE"  allreduce_rs_mv2: decrypted = %lu Rank = %d "COLOR_RESET"\n", dec_count, comm_ptr->rank);
                fflush(stdout);
            }
#endif    
#endif                                                                              
    ...
    if (newrank != -1) {
        if ((HANDLE_GET_KIND(op) != HANDLE_KIND_BUILTIN) || (count < pof2)) {  /* use recursive doubling */
            mask = 0x1;
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
			
            while (mask < pof2) {
				
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-1] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
                ...
                MPIR_PVAR_INC(allreduce, pt2pt_rs, send, count, datatype);
                MPIR_PVAR_INC(allreduce, pt2pt_rs, recv, count, datatype);
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-2] count=%d  mask=%d  pof2 = %d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, pof2,comm_ptr->rank);
				...
				if ((my_node_id == remote_node_id)){ // || (init_phase)){
					...
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-2-1] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					...
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-2-2] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
				} else {
...
if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-3] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
                ...
                mask <<= 1;
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
            }
        } else {
			...
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-1 RSA] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
			...
			//int my_node_id_cyclinc = -1;
			//int remote_node_id_cyclinc = -1;
			if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-2 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
            ...
            while (mask < pof2) {
				...
				//fprintf(stderr,COLOR_MAGENTA"Process %d & %d [%d,%d,%d,%d] count=%d  recv_cnt=%d [RSA_RS]"COLOR_RESET"\n",rank,dst,my_node_id,remote_node_id,my_node_id_cyclinc,remote_node_id_cyclinc,count,recv_cnt);
				
				// MPID_Get_node_id(comm_ptr, MPIR_Rank_list_mapper(comm_ptr, rank + 1), &remote_node_id);
			
				if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-3 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);

				if ((my_node_id == remote_node_id)){
					
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-3-1 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					
					//fprintf(stderr,COLOR_YELLOW"Process %d & %d are in same node: (%d) count=%d   recv_cnt=%d [idx]=%d [RSA_RS]"COLOR_RESET"\n",rank,dst,my_node_id,count,recv_cnt,MPIR_Rank_list_mapper(comm_ptr, dst));	
					...
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-3-2 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					
					//fprintf(stderr,COLOR_CYAN"Process %d & %d are in same node: (%d) count=%d   recv_cnt=%d [idx]=%d [RSA_RS END]"COLOR_RESET"\n",rank,dst,my_node_id,count,recv_cnt,MPIR_Rank_list_mapper(comm_ptr, dst));							 
												 
                } else {
					
					//fprintf(stderr,COLOR_MAGENTA"Process %d & %d are not in same node  count=%d   recv_cnt=%d [RSA_RS]"COLOR_RESET"\n",rank,dst,count,recv_cnt);	
					
					enc_chunk_size = send_cnt*extent + ENC_MSG_TAG_SIZE + NONCE_SIZE;

                    if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-1 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					...
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-2 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					...
					//MPIC_Irecv(large_recv_buffer, enc_chunk_size, MPI_CHAR, MPIR_Rank_list_mapper(comm_ptr, dst),  MPIR_ALLREDUCE_TAG, comm_ptr,&recv_req_ptr);
...
#endif
if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-3 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					...
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-4 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
					
					//MPIC_Isend(large_send_buffer, ciphertext_sendbuf_len+12, MPI_CHAR, MPIR_Rank_list_mapper(comm_ptr, dst), MPIR_ALLREDUCE_TAG, comm_ptr, &send_req_ptr, errflag);
					
					mpi_errno = MPIC_Wait(recv_req_ptr, errflag);
					//if (mpi_errno) MPIR_ERR_POPFATAL(mpi_errno);
					
					//MPIC_Waitall(2, request, status, errflag);
					if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-5 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
...
#endif  
if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-6 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
                ...
                /* This algorithm is used only for predefined ops
				  ...
				  // fprintf(stderr,COLOR_GREEN"Process %d & %d are in same node: (%d) count=%d   recv_cnt=%d [idx]=%d [bfr op]"COLOR_RESET"\n",rank,dst,my_node_id,count,recv_cnt,MPIR_Rank_list_mapper(comm_ptr, dst));	
				...
				//	fprintf(stderr,COLOR_BLUE"Process %d & %d are in same node: (%d) count=%d   recv_cnt=%d [idx]=%d [aft op]"COLOR_RESET"\n",rank,dst,my_node_id,count,recv_cnt,MPIR_Rank_list_mapper(comm_ptr, dst));	
                ...
                if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-4-7 RSA] count=%d  mask=%d  MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus] rank = %d \n", count,mask, comm_ptr->rank);
			...
			//if (count==8192) fprintf(stderr,COLOR_YELLOW"recvbuf[%d] = %.0f  %.0f  %.0f  %.0f  %.0f  %.0f  %.0f  %.0f   host=%d  [mid]"COLOR_RESET"\n",rank, *((float *) (recvbuf+((1000)*extent))), *((float *) (recvbuf+((2000)*extent))), *((float *) (recvbuf+((3000)*extent))),*((float *) (recvbuf+((4000)*extent))), *((float *) (recvbuf+((5000)*extent))), *((float *) (recvbuf+((6000)*extent))), *((float *) (recvbuf+((7000)*extent))),*((float *) (recvbuf+((8000)*extent))),my_node_id);
			...
			if (my_node_id != remote_node_id) {
                ...
                //fprintf(stderr,COLOR_RED"recv_idx=%d   disps[recv_idx]=%d    enc_disps[recv_idx]=%d   rank=%d [bfr]"COLOR_RESET"\n",recv_idx,disps[recv_idx],enc_disps[recv_idx],rank);
            ...
            while (mask > 0) {
				...
				//if (count==8192 &&  rank==3) fprintf(stderr,COLOR_RED"seg_count=%d  all_intra_node=%d  mask=%d"COLOR_RESET"\n",seg_count,all_intra_node,mask);
				
				/* if ((big == 1) && (my_node_id != remote_node_id)) {

                fprintf(stderr,COLOR_YELLOW"recv_idx=%d   disps[recv_idx]=%d    enc_disps[recv_idx]=%d   rank=%d [after]"COLOR_RESET"\n",send_idx,disps[send_idx],enc_disps[send_idx],rank);

					RAND_bytes(large_recv_buffer + enc_disps[send_cnt]  , NONCE_SIZE);
					max_out_len = recv_cnt * extent + ENC_MSG_TAG_SIZE; 
					if (!EVP_AEAD_CTX_seal(global_coll_msg_ctx, (large_recv_buffer + enc_disps[send_cnt] + NONCE_SIZE),
								   &ciphertext_sendbuf_len, max_out_len,
								   (&large_recv_buffer[enc_disps[send_cnt]]), NONCE_SIZE,
								   (char *) recvbuf + disps[send_cnt] * extent, recv_cnt * extent,
								   NULL, 0))
					{
						printf("Error in encryption: RSA rank = %d\n",comm_ptr->rank);
						fflush(stdout);
				...
				//seg_size = send_cnt;                                      
                //seg_size_char = (unsigned long) send_cnt * extent; */
				...
				if ((my_node_id == remote_node_id)){
					...
					if ((same_node == 0)  &&  (all_intra_node == 0)) {
						...
						//if (count==8192 &&  rank==3) fprintf(stderr,COLOR_RED"step_power=%d [Last-In]"COLOR_RESET"\n",step_power);
					...
					//fprintf(stderr,COLOR_YELLOW"Process %d & %d are not in same node count=%d  step_power=%d recv_cnt=%d  mask=%d [RSA - ALG]"COLOR_RESET"\n",rank,dst,count,step_power,recv_cnt,mask);	
					
					//MPIC_Irecv(large_recv_buffer + (enc_disps[recv_idx]), enc_chunk_size, MPI_CHAR, MPIR_Rank_list_mapper(comm_ptr, dst),  MPIR_ALLREDUCE_TAG, comm_ptr,&recv_req_ptr);
					
					//MPIC_Isend(large_recv_buffer + (enc_disps[send_idx]), enc_chunk_size, MPI_CHAR, MPIR_Rank_list_mapper(comm_ptr, dst), MPIR_ALLREDUCE_TAG, comm_ptr, &send_req_ptr, errflag);
					...
					if (step_power > 0)
					{
						...
						//if (count==8192 &&  rank==3) fprintf(stderr,COLOR_RED"step_power=%d"COLOR_RESET"\n",step_power);
						
						for (i=0; i<step_power; i++){
							
							//if (count==8192) fprintf(stderr,COLOR_GREEN"rank[%d]=%d  step_power=%d  basic_seg_size=%d  basic_cipher_size=%d  last_recv_idx=%d disp=%d  enc_disp=%d"COLOR_RESET"\n",rank,i,step_power,basic_seg_size,basic_cipher_size,last_recv_idx,((disps[last_recv_idx]*extent)+i*basic_seg_size),(enc_disps[last_recv_idx])+ i * basic_cipher_size);
			...
			if ((same_node == 0) && (all_intra_node == 0)) {
				...
				//if (count==8192) fprintf(stderr,COLOR_RED"rank[%d]=%d  step_power=%d  last_recv_idx=%d disp=%d  enc_disp=%d[Last-out]"COLOR_RESET"\n",rank,i,step_power,last_recv_idx,disps[last_recv_idx],enc_disps[last_recv_idx]);
					
				for (i=0; i<step_power; i++){
					
					//if (count==8192) fprintf(stderr,COLOR_RED"rank[%d]=%d  step_power=%d  basic_seg_size=%d  basic_cipher_size=%d  last_recv_idx=%d disp=%d  enc_disp=%d"COLOR_RESET"\n",rank,i,step_power,basic_seg_size,basic_cipher_size,last_recv_idx,((disps[last_recv_idx]*extent)+i*basic_seg_size),(enc_disps[last_recv_idx])+ i * basic_cipher_size);
			...
			if (SHOW_FINAL_RESULT && count==8192)  fprintf(stderr,COLOR_CYAN"recvbuf[%d] = %.0f  %.0f  %.0f  %.0f  %.0f  %.0f  %.0f  %.0f    host=%d  [end]"COLOR_RESET"\n",rank, *((float *) (recvbuf+((1000)*extent))), *((float *) (recvbuf+((2000)*extent))), *((float *) (recvbuf+((3000)*extent))),*((float *) (recvbuf+((4000)*extent))), *((float *) (recvbuf+((5000)*extent))), *((float *) (recvbuf+((6000)*extent))), *((float *) (recvbuf+((7000)*extent))),*((float *) (recvbuf+((8000)*extent))),my_node_id);
        }
    }
	
	if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-7] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
            ...
            if ((my_node_id == remote_node_id)){
                ...
                mpi_errno = MPIC_Send(recvbuf, count,
                 ...
                 mpi_errno = MPIC_ENC_NAIVE_Send(recvbuf, count,
                                     datatype, MPIR_Rank_list_mapper(comm_ptr, rank - 1),
                                     MPIR_ALLREDUCE_TAG, comm_ptr, errflag);
// fprintf(stderr,"[*] rank = %d  [RSA-SND End]\n",comm_ptr->rank);         
            }                         
#if 0                                                                          
            temp_data = count * extent;
            large_send_buffer[0] = (temp_data >> 24) & 0xFF;
            ...
            if (!EVP_AEAD_CTX_seal(global_coll_msg_ctx, (large_send_buffer + MSG_HEADER_SIZE),
                ...
                {
                    if (ENABLE_SECURE_DEBUG)
						fprintf(stderr,COLOR_GREEN"allreduce_rs_mv2: ciphertext_sendbuf_len = %d Rank = %d "COLOR_RESET"\n", ciphertext_sendbuf_len, comm_ptr->rank);
                    //fflush(stdout);
                }
#endif                           
            ...
            if ((my_node_id == remote_node_id)){
                ...
                mpi_errno = MPIC_ENC_NAIVE_Recv(recvbuf, count,
                                  ...
                                  MPIR_ALLREDUCE_TAG, comm_ptr,
                                  MPI_STATUS_IGNORE, errflag);
// fprintf(stderr,"[*] rank = %d  [RSA-RCV END]\n",comm_ptr->rank);         
            }
            ...
            {
                if (ENABLE_SECURE_DEBUG)
						fprintf(stderr,COLOR_BLUE"  allreduce_rs_mv2: decrypted = %lu Rank = %d "COLOR_RESET"\n", dec_count, comm_ptr->rank);
                //fflush(stdout);
            }
#endif      
#endif                                                                         
	...
	/* if (count==32768) {
		printf("recvbuf[%d] = %f  %f  %f  rank=%d [RSA]\n",count, *((float *) (recvbuf+((1000)*extent))), *((float *) (recvbuf+((count/2)*extent))), *((float *) (recvbuf+((count-1)*extent))),rank);
		printf("tmp_buf[%d] = %f  %f  %f  rank=%d [RSA]\n",count, *((float *) (tmp_buf+((1)*extent))), *((float *) (tmp_buf+((count/2)*extent))), *((float *) (tmp_buf+((count-1)*extent))),rank);
		fflush(stdout);
	} */

    if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-8] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
...
if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-9] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
  fn_exit:
    MPIU_CHKLMEM_FREEALL();
    MPIR_TIMER_END(coll,allreduce,shm_rs);
	if (PRINT_RSA_FUN_NAME)  fprintf(stderr,"[3-10] rank = %d count=%d MPIR_Allreduce_pt2pt_rs_MV2 [NAIVE Plus]\n", comm_ptr->rank, count);
    return (mpi_errno);
...
int MPIR_Allreduce_two_level_MV2(const void *sendbuf,
                             ...
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    if (super_node==1 && security_approach>0)
                             ...
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    /* if (init_phase) {
        ...
        int namelen;
        gethostname(hostname, &namelen);
		printf("[allreduce rank = %d host = %s] Func: MPIR_Allreduce_two_level_MV2_SEC_Super_Node  count = %d [SUPER_NODE - Switch to Original]\n",comm_ptr->rank,hostname,count);fflush(stdout);
        mpi_errno = MPIR_Allreduce_original_two_level_MV2(sendbuf, recvbuf, count,
	...
	while (leader_cnt >= 1){    
		...
		if (comm_ptr->dev.ch.rank_list == NULL || is_commutative != 1) {
			...
			//fprintf(stderr,"[r=%d]: SUPER_NODE : rank_list == NULL[3]\n",comm_ptr->rank); 
...
// if (PRINT_SUPER)     fprintf(stderr,"[r=%d , %s]: SUPER_NODE  list_rank=%d   node_rank=%d  node=%d  leader_cnt=%d   Sender_cnt=%d   msg_recv_cnt=%d [4]\n",comm_ptr->rank,hostname,list_rank,node_rank,node,leader_cnt,Sender_cnt,msg_recv_cnt);            		
                             ...
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
	...
	// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [1]\n",comm_ptr->rank);
...
// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [4]\n",comm_ptr->rank);
        if ((MV2_Allreduce_intra_function == &MPIR_Allreduce_reduce_shmem_MV2) || 
              (MV2_Allreduce_intra_function == &MPIR_Allreduce_reduce_p2p_MV2) ) {
        mpi_errno =
...
// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [5]\n",comm_ptr->rank);	
...
/* *************************** Mohsen ************************** */		
		...
		MPIU_THREADPRIV_GET;
// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [2]\n",comm_ptr->rank);
		/* check if multiple threads are calling this collective function */
		MPIDU_ERR_CHECK_MULTIPLE_THREADS_ENTER(comm_ptr);
		...
		// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [3]\n",comm_ptr->rank);
        ...
        tmp_buf = (void *) ((char *) tmp_buf - true_lb);   
			...
			if (PRINT_FUN_NAME)     fprintf(stderr,"[rank=%d]: two_level_MV2 [R = 6] Count = %d\n",comm_ptr->rank,count);
            ...
            //if (SHOW_FINAL_RESULT) fprintf(stderr,"tmp_buf[%d] = %f  %f  %f  %f  %f  %f  %f  %f  count=%d  [aft Allgather]"COLOR_RESET"\n",comm_ptr->rank, *((float *) (tmp_buf+extent)),*((float *) (tmp_buf+step+(count*extent))),*((float *) (tmp_buf+2*(step+count*extent))),*((float *) (tmp_buf+3*(step+count*extent))),*((float *) (tmp_buf+4*(step+count*extent))),*((float *) (tmp_buf+5*(step+count*extent))),*((float *) (tmp_buf+6*(step+count*extent))),*((float *) (tmp_buf+7*(step+count*extent))),count);
            ...
            for (i=0; i<leader_size; i++){
                ...
                /* tmp_buf contains data received in this step.
                   recvbuf contains data accumulated so far */    
// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [7]\n",comm_ptr->rank);
#if defined(HAVE_CXX_BINDING)
            if (is_cxx_uop) {
                (*MPIR_Process.cxx_call_op_fn) (local_buf, recvbuf,
                                                count, datatype, uop);
...
// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [8]\n",comm_ptr->rank);			
        ...
        else 
        {
            ...
            // if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [9]\n",comm_ptr->rank);
    ...
    {
		// local_rank != 0
        // if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [10-1]\n",comm_ptr->rank);
		...
		// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: two_level_MV2 [10-2]\n",comm_ptr->rank);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
                MPIR_ERR_SET(mpi_errno, MPI_ERR_OTHER, "**fail");
...
//if (PRINT_FUN_NAME && (comm_ptr->rank == 12 || comm_ptr->rank == 34 || comm_ptr->rank == 7 || comm_ptr->rank == 8))     fprintf(stderr,"The process took %lf seconds to run Count = %d  rank = %d\n",end - start,count,comm_ptr->rank);
//if (comm_ptr->rank == 0)     printf("%lf     seconds to run Count = %d  rank = %d\n",end - start,count,comm_ptr->rank);
...
int MPIR_Allreduce_original_two_level_MV2(const void *sendbuf,
                             void *recvbuf,
                             int count,
                             MPI_Datatype datatype,
                             MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (PRINT_FUN_NAME || DEBUG_INIT_FILE && (comm_ptr->rank == 0)) {		
        ...
        int namelen;
        gethostname(hostname, &namelen);
        printf("[allreduce rank = %d host = %s] Func: MPIR_Allreduce_original_two_level_MV2  count = %d\n",comm_ptr->rank,hostname,count);fflush(stdout);}
#endif     
    MPIR_TIMER_START(coll,allreduce,2lvl);
    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_2lvl, 1);
    int mpi_errno = MPI_SUCCESS;
...
//if (comm_ptr->rank == 0)     printf("%lf     seconds to run Count = %d  rank = %d\n",end - start,count,comm_ptr->rank);
    ...
    if (SHOW_FINAL_RESULT && init_phase==0 && count>16  && count!=65) {    
        ...
        if (match==1) {
            fprintf(stderr,"[Original_two_level Rank = %d] is match [value = %f] host=%s  count=%d  [end]\n",comm_ptr->rank,*((float *) (recvbuf+extent)),hostname1,count);
        } else       
            fprintf(stderr,"[Original_two_level Rank = %d] recvbuf = %f  %f  %f  %f  %f  %f  %f  %f  count=%d   host=%s  [end]\n",comm_ptr->rank, *((float *) (recvbuf+extent)),*((float *) (recvbuf+step)),*((float *) (recvbuf+2*step)),*((float *) (recvbuf+3*step)),*((float *) (recvbuf+4*step)),*((float *) (recvbuf+5*step)),*((float *) (recvbuf+6*step)),*((float *) (recvbuf+7*step)),count,hostname1);
    }
#endif		
                                          ...
                                          MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
	...
	// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 Count = %d [1]\n",comm_ptr->rank,count);
...
// if (PRINT_SUPER)     fprintf(stderr,"[r=%d , %s]: SUPER_NODE  list_rank=%d   node_rank=%d  node=%d  leader_cnt=%d   Sender_cnt=%d   msg_recv_cnt=%d [4]\n",comm_ptr->rank,hostname,list_rank,node_rank,node,leader_cnt,Sender_cnt,msg_recv_cnt);            		
 ...
 // if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 Count = %d [9]\n",comm_ptr->rank,count);
                                          ...
                                          MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    nbytes = (MPIDI_msg_sz_t) (count) * (type_size);
    MPI_Aint extent = type_size;
	//if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 [0]\n",comm_ptr->rank);
    /* Step 1. Socket leaders do an intra-socket reduce using shared memory*/
    ...
    if(local_rank == 0 && leader_commptr->local_size > 1)
    {
        ...
        if (Allgather_Reduce){
	// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 Count = %d [2]\n",comm_ptr->rank,count);
		...
		// if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 Count = %d [3]\n",comm_ptr->rank,count);
        ...
        tmp_buf = (void *) ((char *) tmp_buf - true_lb);   
			...
			// if (PRINT_FUN_NAME)     fprintf(stderr,"[rank=%d]: Socket_two_level_MV2 Count = %d [4]\n",comm_ptr->rank,count);
            MPIR_Allgather_MV2(recvbuf, count, datatype, tmp_buf, count, datatype, leader_commptr, errflag);
            //MPIR_Allgather_RD_MV2(recvbuf, count, datatype, tmp_buf, count, datatype, leader_commptr, errflag);
            ...
            for (i=0; i<leader_size; i++){
                ...
                /* tmp_buf contains data received in this step.
                   recvbuf contains data accumulated so far */    
if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 Count = %d [7]\n",comm_ptr->rank,count);
#if defined(HAVE_CXX_BINDING)
            if (is_cxx_uop) {
                (*MPIR_Process.cxx_call_op_fn) (local_buf, recvbuf,
                                                count, datatype, uop);
 ...
 // if (PRINT_FUN_NAME)     fprintf(stderr,"[r=%d]: Socket_two_level_MV2 Count = %d [8]\n",comm_ptr->rank,count);
                       ...
                       MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (comm_ptr->dev.ch.shmem_coll_ok == 1 && comm_ptr->dev.ch.is_uniform) {
        ...
        if (mv2_allred_use_ring == 1 && mv2_allreduce_ring_algo_threshold <=  nbytes && local_size <= 16) {
			...
			if (DEBUG_INIT_FILE) printf("[1*]  MPIR_Allreduce_index_tuned_intra_MV2  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
					           

			//return MPIR_Allreduce_pt2pt_reduce_scatter_allgather_MV2(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
            ...
            mpi_errno = MPIR_Allreduce_pt2pt_ring_wrapper_MV2(sendbuf, recvbuf, count, datatype,
				...
				if (DEBUG_INIT_FILE || PRINT_Ring_FUN_NAME) printf("[2*]  MPIR_Allreduce_index_tuned_intra_MV2  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
                       ...
                       MPI_Op op, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
	...
	// int max_eager = comm_ptr->dev.eager_max_msg_sz;
	// if (PRINT_FUN_NAME) printf("max_eager = %d  [Rank = %d]\n",max_eager,comm_ptr->rank);
    ...
    if(mv2_use_old_allreduce == 1){
        ...
        if (mv2_use_indexed_tuning || mv2_use_indexed_allreduce_tuning) {
			if (DEBUG_INIT_FILE) printf("[1] MPIR_Allreduce_MV2  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
            mpi_errno = MPIR_Allreduce_index_tuned_intra_MV2(sendbuf, recvbuf, count,
                    datatype, op, comm_ptr, errflag);
if (DEBUG_INIT_FILE || PRINT_Ring_FUN_NAME) printf("[2] MPIR_Allreduce_MV2  count = %d  [Rank = %d]\n",count,comm_ptr->rank);					
        }
        else {
	...
	if (DEBUG_INIT_FILE || PRINT_Ring_FUN_NAME) printf("[3] MPIR_Allreduce_MV2  count = %d  [Rank = %d]\n",count,comm_ptr->rank);

    MPIR_T_PVAR_COMM_TIMER_END(MV2, mv2_coll_timer_allreduce_subcomm, comm_ptr);
if (DEBUG_INIT_FILE || PRINT_Ring_FUN_NAME) printf("[End] MPIR_Allreduce_MV2  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
    return (mpi_errno);
}
...
int MPIR_Allreduce_pt2pt_ring_wrapper_MV2(const void *sendbuf, void *recvbuf,
    ...
    int remaining_count = count - new_count;
		...
		if (sendbuf != MPI_IN_PLACE){
			//if (DEBUG_INIT_FILE || PRINT_Ring_FUN_NAME) printf("[1] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
        mpi_errno = MPIR_Allreduce_pt2pt_ring_MV2(sendbuf, recvbuf, new_count, datatype,
            op, comm_ptr, errflag);
			//if (DEBUG_INIT_FILE || PRINT_Ring_FUN_NAME) printf("[2] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
			
		}
    else  {
		//if (DEBUG_INIT_FILE) printf("[3] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
        mpi_errno = MPIR_Allreduce_pt2pt_ring_inplace_MV2(sendbuf, recvbuf, new_count, datatype,
            op, comm_ptr, errflag);
			//if (DEBUG_INIT_FILE) printf("[4] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
	}
	//if (DEBUG_INIT_FILE) printf("[5] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
    if (mpi_errno != MPI_SUCCESS || (remaining_count == 0)) 
        return mpi_errno;
    //if (DEBUG_INIT_FILE) printf("[6] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
    /* now Allreduce the remaining count */ 
    if (sendbuf != MPI_IN_PLACE){
		//if (DEBUG_INIT_FILE) printf("[7] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
		...
		//if (DEBUG_INIT_FILE) printf("[8] ring_wrapper  count = %d  [Rank = %d]\n",count,comm_ptr->rank);
...
//            if (rank_index < dst) {
    ...
    comm_ptr->rank,CONCUR_INTER_METHOD,choping_sz,count);
        ...
        if (Print_Ring && SHOW_FINAL_RESULT && count==8192) {
            ...
            fprintf(stderr,COLOR_YELLOW"[End RD Rank = %d]  print_chunk = %d  %d  %d  %d  %d  %d  %d  %d  count=%d   host=%s  [end]"COLOR_RESET"\n",rank,*((int *) (print_chunk)) , *((int *) (print_chunk+step)),*((int *) (print_chunk+2*step)),*((int *) (print_chunk+3*step)),*((int *) (print_chunk+4*step)),*((int *) (print_chunk+5*step)),*((int *) (print_chunk+6*step)),*((int *) (print_chunk+7*step)),count,hostname1);
	...
	if (ppn > 1) {
		...
		if (CONCUR_AllGTHER_METHOD == 1)  // Ring-method
		{
            ...
            fprintf(stderr,COLOR_YELLOW"[Begin Algather Rank = %d]  print_chunk = %d  %d  %d  %d  %d  %d  %d  %d %d  %d  %d  %d count=%d   host=%s  [end]"COLOR_RESET"\n",rank,*((int *) (print_chunk)) , *((int *) (print_chunk+step)),*((int *) (print_chunk+2*step)),*((int *) (print_chunk+3*step)),*((int *) (print_chunk+4*step)),*((int *) (print_chunk+5*step)),*((int *) (print_chunk+6*step)),*((int *) (print_chunk+7*step)),*((int *) (print_chunk+8*step)),*((int *) (print_chunk+9*step)),*((int *) (print_chunk+10*step)),*((int *) (print_chunk+11*step)),count,hostname1);
...
//    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_ring_colls, 1);
    ...
    // if (SHOW_FINAL_RESULT && count==8192)  fprintf(stderr," Ring_rank (cyclic/blocked) : rank = %d   rank_index = %d\n",rank,rank_index);
    ...
    // if (count==8192) fprintf(stderr, COLOR_GREEN"pof2 = %d  rem = %d"COLOR_RESET"\n",pof2,rem);


    if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[1] Unsec-ML-Ring [r=%d]\n",rank);
    ...
    if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[2] Unsec-ML-Ring [r=%d]\n",rank);
#if 1	
	if (ppn > 1) {
        /* do a Intra-node reduce-scatter followed by allgather */
        MPIU_Assert(sendbuf != MPI_IN_PLACE);
		...
		//fprintf(stderr, "Rank=%d  shmem_rank=%d  rank_index=%d  right=%d  left=%d   host=%s  node_id=%d\n",rank,shmem_rank,rank_index,right,left,hostname1,node_id); 	

    //if (PRINT_FUN_NAME)     fprintf(stderr,"CONCUR_RS_METHOD = %d [r=%d]\n",CONCUR_RS_METHOD,rank);        
    ...
    if (CONCUR_RS_METHOD == 1) { // Pt2pt-method
		...
		{
			//if (PRINT_FUN_NAME)     fprintf(stderr,"[2.1] Unsec-ML-Ring [r=%d]\n",rank);
			//if (SHOW_FINAL_RESULT && count==8192 && rank==5) fprintf(stderr,"[%d] shmem_buffer is used at Reduce-Scatter\n",rank_index);            
...
#if 0
            if (SHOW_FINAL_RESULT && count==8192 && rank==1){
                fprintf(stderr,"\nshmem_buffer\n");

                for (i=0; i<count; i++){
                        if ( (i % 500) == 0) fprintf(stderr," -- %d-- %f ",i,*( (float *) (shmem_buffer+(2*srep2)+(i*extent)) ) );
                        else fprintf(stderr, " %f ",*((float *) (shmem_buffer+(2*srep2)+(i*extent))));
                    }
...
/* ********** All ranks contribute on RS *********** */			

			recv_chunk  = sendbuf + (shmem_rank * chunk_size_intra);

            for (i = 0; i < ppn; i++) {

                if (i != shmem_rank) {
                                 
                    comp_chunk  = shmem_buffer + (i * count * type_size) + (shmem_rank * chunk_size_intra);                    
            ...
            if (leader_cnt == ppn)    {
                ...
                mpi_errno = MPIR_Localcopy((void*)((char*) (sendbuf + (shmem_rank*chunk_size_intra)) ), (count / ppn), datatype, 
                                    (void*)((char*) (tmp_buf + (shmem_rank*chunk_size_intra))), (count / ppn), datatype); 

				if (SHOW_FINAL_RESULT && count==8192) fprintf(stderr,"leader_cnt == ppn\n");							
            } else {
                mpi_errno = MPIR_Localcopy((void*)((char*) (sendbuf + (shmem_rank*chunk_size_intra)) ), (count / ppn), datatype, 
                                    (void*)((char*) (shmem_buffer + (shmem_rank*chunk_size_intra))), (count / ppn), datatype);
	...
	// if (SHOW_FINAL_RESULT && count==8192) fprintf(stderr, COLOR_YELLOW"tmp_buf[%d] = %f  %f  %f  %f  %f  %f  %f  %f  rank=%d   host=%s  [begin]"COLOR_RESET"\n",count, *((float *) (tmp_buf+step)),*((float *) (tmp_buf+2*step)),*((float *) (tmp_buf+3*step)),*((float *) (tmp_buf+4*step)),*((float *) (tmp_buf+5*step)),*((float *) (tmp_buf+6*step)),*((float *) (tmp_buf+7*step)),*((float *) (tmp_buf+8*step)),rank,hostname1);

if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[3] Unsec-ML-Ring [r=%d]\n",rank);
    ...
    if (Print_Ring && SHOW_FINAL_RESULT && count==8192) {
            ...
            fprintf(stderr,COLOR_YELLOW"[Begin Rank = %d]  print_chunk = %d  %d  %d  %d  %d  %d  %d  %d  count=%d   host=%s  [end]"COLOR_RESET"\n",rank,*((int *) (print_chunk)) , *((int *) (print_chunk+step)),*((int *) (print_chunk+2*step)),*((int *) (print_chunk+3*step)),*((int *) (print_chunk+4*step)),*((int *) (print_chunk+5*step)),*((int *) (print_chunk+6*step)),*((int *) (print_chunk+7*step)),count,hostname1);
...
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[4] Unsec-ML-Ring [r=%d]\n",rank);
    //if (SHOW_FINAL_RESULT && count==8192) fprintf(stderr,"Ran_leader = %d  host = % s chunk_size_leader = %d\n",rank,hostname1,chunk_size_leader); 
    if (CONCUR_INTER_METHOD == 1)  // RD-method
    {
        ...
        void * large_recv_buffer2 = recv_chunk;
        void * large_send_buffer2 = send_chunk;
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5] Unsec-ML-Ring [r=%d]\n",rank);

#if 0
        if (CONCUR_AllGTHER_METHOD == 2){ 
            recv_chunk  = shmem_buffer + (shmem_rank * chunk_size_intra);    
            // if (SHOW_FINAL_RESULT && count==8192) fprintf(stderr,"[%d] shmem_buffer is  used at RD-Inter \n",rank_index);
        }
        else 
            recv_chunk  = recvbuf + (shmem_rank * chunk_size_intra);
#endif 

        while (mask < pof2) {
			if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-1] Unsec-ML-Ring   Unsec-ML-Ring [r=%d]\n",rank);
            
            dst_index = rank_index ^ mask;

            dst  = comm_ptr->dev.ch.rank_list[dst_index];
			
			if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-2] Unsec-ML-Ring   mask = %d  dst = %d  shmem_rank = %d    chunk_size_leader = %d  [r=%d]\n",mask,dst,shmem_rank,chunk_size_leader,rank);

            //if (count==8192) fprintf(stderr,"rank = %d  dst = %d  mask = %d \n",rank_index,dst,mask);

            mpi_errno = MPID_Irecv(large_recv_buffer2, chunk_enc_size, MPI_CHAR, dst, 11, comm_ptr, context_id, &recv_req_ptr);  
			//if (mpi_errno) MPIR_ERR_POP(mpi_errno);	
#if 0			
			if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-3] Unsec-ML-Ring [r=%d]\n",rank);

			RAND_bytes(large_send_buffer2, NONCE_SIZE);
			if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-4] Unsec-ML-Ring [r=%d]\n",rank);
			max_out_len = chunk_size_leader+ENC_MSG_TAG_SIZE; 
			if (!EVP_AEAD_CTX_seal(global_coll_msg_ctx, (large_send_buffer2+NONCE_SIZE),
										   &ciphertext_sendbuf_len, max_out_len,
										   (&large_send_buffer2[0]), NONCE_SIZE,
										   (char *)send_chunk, chunk_size_leader,
										   NULL, 0))
						{
							printf("Error in encryption: Concur-Inter: RD  rank = %d\n",comm_ptr->rank);
            ...
            if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-5] Unsec-ML-Ring [r=%d]\n",rank);
			mpi_errno = MPID_Isend(large_send_buffer2, chunk_enc_size, MPI_CHAR, dst, 11, comm_ptr, context_id, &send_req_ptr);                       
			//if (mpi_errno) MPIR_ERR_POP(mpi_errno);
			if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-6] Unsec-ML-Ring [r=%d]\n",rank);
			mpi_errno = MPIC_Wait(recv_req_ptr, errflag);
			//if (mpi_errno) MPIR_ERR_POPFATAL(mpi_errno);
			if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-7] Unsec-ML-Ring [r=%d]\n",rank);

#if 0            
			if (!EVP_AEAD_CTX_open(global_coll_msg_ctx, (char *)recv_chunk,
							   &dec_count, chunk_size_leader,
							   (&large_recv_buffer2[0]), NONCE_SIZE,
							   large_recv_buffer2+(NONCE_SIZE), (chunk_size_leader + ENC_MSG_TAG_SIZE),
							   NULL, 0))
			{
				printf("Decryption error at Concur-Inter: RD  rank = %d source = %d  mask = %d  count = %d\n",comm_ptr->rank, dst,mask,count);
				fflush(stdout);
            }
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-8] Unsec-ML-Ring [r=%d]\n",rank);
#endif
            // send_chunk  = recvbuf + (shmem_rank * chunk_size_intra);

            recv_chunk  = tmp_buf + (shmem_rank * chunk_size_leader);

            send_chunk  = rep_chunk + (shmem_rank * chunk_size_leader);
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-9] Unsec-ML-Ring [r=%d]\n",rank);
            /* if (CONCUR_AllGTHER_METHOD == 2)
                send_chunk  = shmem_buffer + (shmem_rank * chunk_size_intra);
            else 
                send_chunk  = recvbuf + (shmem_rank * chunk_size_intra); */
...
#endif
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-10] Unsec-ML-Ring [r=%d]\n",rank);
            mpi_errno = MPIC_Wait(send_req_ptr, errflag);
            if (mpi_errno) MPIR_ERR_POP(mpi_errno);
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-11] Unsec-ML-Ring [r=%d]\n",rank);
            MPID_Request_release(send_req_ptr);
            MPID_Request_release(recv_req_ptr);
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[5-12] Unsec-ML-Ring [r=%d]\n",rank);
            mask <<= 1;
...
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[6] Unsec-ML-Ring [r=%d]\n",rank);
    }
    ...
    else if (CONCUR_INTER_METHOD == 2)  // RSA-method
    {
		...
		/* if (PRINT_FUN_NAME || DEBUG_INIT_FILE) 
			fprintf(stderr,"[rank = %d  CONCUR_RS_METHOD = %d  CONCUR_INTER_METHOD = %d  CONCUR_AllGTHER_METHOD = %d  leader_cnt = %d  count=%d] Func: MPIR_Allreduce_pt2pt_ring_MV2 [UNSEC_ALLREDUCE_MULTI_LEADER]\n",
			comm_ptr->rank,CONCUR_RS_METHOD,CONCUR_INTER_METHOD,CONCUR_AllGTHER_METHOD,leader_cnt,count); */
		...
		if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[*7-1] Unsec-ML-Ring [r=%d]\n",rank);
...
//            if (rank_index < dst) {
            ...
            if (SHOW_FINAL_RESULT && count==8192 && (rank==0 || rank==1)) fprintf(stderr,"[rank = %d    dst = %d   mask = %d]\n",rank,dst,mask);
        ...
        if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[*7-9] Unsec-ML-Ring [r=%d]\n",rank);
    ...
    {    
		...
		//if (count==8192) fprintf(stderr,COLOR_YELLOW"Rank=%d   rank_index=%d  left_Inter=%d  right_Inter=%d  host=%s  node_id=%d  inter_prefix=%d  chunk_size_inter=%d"COLOR_RESET"\n",rank,rank_index,left_Inter,right_Inter,hostname1,node_id,inter_prefix,chunk_size_inter); 
        ...
        for (i = 1; i < node; i++) {
            ...
            comp_chunk  = tmp_buf + ((node_id - i + node) % node)*chunk_size_inter + inter_prefix;   
 			...
 			/* mpi_errno = MPIC_ENC_NAIVE_Sendrecv(send_chunk, chunk_count_inter, datatype,
												 right, 11,
												 recv_chunk, chunk_count_inter, datatype, 
												 left, 11, 
												 comm_ptr, MPI_STATUS_IGNORE, errflag); 
...
#if 0
			RAND_bytes(large_recv_buffer2+send_enc_chunk, NONCE_SIZE);
			max_out_len = chunk_size_inter+ENC_MSG_TAG_SIZE; 
			if (!EVP_AEAD_CTX_seal(global_coll_msg_ctx, (large_recv_buffer2+(send_enc_chunk + NONCE_SIZE)),
										   &ciphertext_sendbuf_len, max_out_len,
										   (&large_recv_buffer2[send_enc_chunk]), NONCE_SIZE,
										   (char *)send_chunk, chunk_size_inter,
										   NULL, 0))
						{
							printf("Error in encryption: Red-Scat step  rank = %d\n",comm_ptr->rank);
			...
			if (!EVP_AEAD_CTX_open(global_coll_msg_ctx, (char *)recv_chunk,
							   &dec_count, chunk_size_inter,
							   (&large_recv_buffer2[recv_enc_chunk]), NONCE_SIZE,
							   large_recv_buffer2+(recv_enc_chunk+NONCE_SIZE), (chunk_size_inter + ENC_MSG_TAG_SIZE),
							   NULL, 0))
			{
				printf("Decryption error at Red-Scat step: rank = %d source =%d  iter = %d  count = %d\n",comm_ptr->rank, left_Inter,i,count);
				fflush(stdout);
        ...
        for (i = 2; i < node; i++) {		
			...
			if (!EVP_AEAD_CTX_open(global_coll_msg_ctx, (char *)send_chunk,
										   &dec_count, chunk_size_inter,
										   (&large_recv_buffer2[send_enc_chunk]), NONCE_SIZE,
										   large_recv_buffer2+(send_enc_chunk+NONCE_SIZE), (chunk_size_inter + ENC_MSG_TAG_SIZE),
										   NULL, 0))
						{
							printf("Decryption error inside allgather step rank = %d source =%d  iter = %d  count = %d\n",comm_ptr->rank, left_Inter,i,count);
							fflush(stdout);
    ...
    // if (count==8192) printf("recvbuf[%d] = %f  %f  %f  %f  %f  %f  %f  %f  rank=%d   host=%s  [mid]"COLOR_RESET"\n",rank, *((float *) (recvbuf+((1000)*extent))), *((float *) (recvbuf+((2000)*extent))), *((float *) (recvbuf+((3000)*extent))),*((float *) (recvbuf+((4000)*extent))), *((float *) (recvbuf+((5000)*extent))), *((float *) (recvbuf+((6000)*extent))), *((float *) (recvbuf+((7000)*extent))),*((float *) (recvbuf+((8000)*extent))),count,hostname1);
	...
	if (ppn > 1) {
        ...
        if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7-0] Unsec-ML-Ring [r=%d]\n",rank);

		if (CONCUR_AllGTHER_METHOD == 1)  // Ring-method
		{
			int i = 0;

            if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7] Unsec-ML-Ring [r=%d]\n",rank);
...
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7-2] Unsec-ML-Ring [r=%d]\n",rank);
				mpi_errno = MPID_Irecv(recv_chunk, chunk_count_intra, datatype, left, 11, comm_ptr,
						context_id, &recv_req_ptr);
				if (mpi_errno) MPIR_ERR_POP(mpi_errno);
				if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7-3] Unsec-ML-Ring [r=%d]\n",rank);
				send_chunk = ((shmem_rank - (i-2) + ppn)%ppn)*chunk_size_intra + recvbuf;     
            ...
            step = tempp;
				...
				if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7-4] Unsec-ML-Ring [r=%d]\n",rank);
				mpi_errno = MPID_Isend(send_chunk, chunk_count_intra, datatype, right, 11,
									   comm_ptr, context_id, &send_req_ptr);
				if (mpi_errno) MPIR_ERR_POP(mpi_errno);
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7-5] Unsec-ML-Ring [r=%d]\n",rank);
				mpi_errno = MPIC_Wait(recv_req_ptr, errflag);
				if (mpi_errno) MPIR_ERR_POPFATAL(mpi_errno);
				// mpi_errno = MPIC_Wait(send_req_ptr, errflag);
				// if (mpi_errno) MPIR_ERR_POP(mpi_errno);
if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[7-6] Unsec-ML-Ring [r=%d]\n",rank);
				// MPID_Request_release(send_req_ptr);
				MPID_Request_release(recv_req_ptr);

                if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[8] Unsec-ML-Ring [r=%d]\n",rank);

	#if 0            
				send_chunk_ar = ((shmem_rank - (i-2) + ppn)%ppn)*chunk_size_intra + recvbuf;
				
				if (rank==30 && count==8192) fprintf(stderr,COLOR_GREEN"[%d] [%d %d]  left = %d right = %d  ch-s= %d t = %d ppn = %d  recvbuf[%d] = %f  %f  %f   recv_chunk = %d [i=%d]\n",rank,shmem_rank,chunk_count_intra,left,right,chunk_size_intra, type_size,ppn, ((shmem_rank - (i-2) + ppn)%ppn)*chunk_count_intra, *((float *) (send_chunk_ar)) , *((float *) (send_chunk_ar+1*extent)) , *((float *) (send_chunk_ar+(chunk_count_intra-1)*extent)) , ((shmem_rank - (i-1) + ppn)%ppn)*chunk_count_intra , i);

				if (rank==31 && count==8192) fprintf(stderr,COLOR_YELLOW"[%d] [%d %d]  left = %d right = %d  recvbuf[%d] = %f  %f  %f   recv_chunk = %d [i=%d]\n",rank,shmem_rank,chunk_count_intra,left,right, ((shmem_rank - (i-2) + ppn)%ppn)*chunk_count_intra, *((float *) (send_chunk_ar)) , *((float *) (send_chunk_ar+1*extent)) , *((float *) (send_chunk_ar+(chunk_count_intra-1)*extent)) , ((shmem_rank - (i-1) + ppn)%ppn)*chunk_count_intra , i);

				// if (rank==30 && count==8192) fprintf(stderr,COLOR_RED"recvbuf[%d] = %f  %f  %f  %f  %f  %f  %f  %f  i=%d   host=%s  [alg]"COLOR_RESET"\n",rank, *((float *) (recvbuf+step)),*((float *) (recvbuf+2*step)),*((float *) (recvbuf+3*step)),*((float *) (recvbuf+4*step)),*((float *) (recvbuf+5*step)),*((float *) (recvbuf+6*step)),*((float *) (recvbuf+8190*extent)),*((float *) (recvbuf+8191*extent)),i,hostname1);
	#endif            
			}
		...
		{
			 if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[9] Unsec-ML-Ring [r=%d]\n",rank);
			//if (SHOW_FINAL_RESULT && count==8192 && rank==5) fprintf(stderr,"[%d] shmem_buffer is used at Allgather\n",rank_index);
            ...
            if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[9-1] Unsec-ML-Ring [r=%d]\n",rank);
			...
			if(comm_ptr->dev.ch.is_global_block==1){
				//Blocked
                if (SHOW_FINAL_RESULT && count==8192) fprintf(stderr,COLOR_GREEN"[%d] is_global_block is 1\n"COLOR_RESET,rank_index);
				mpi_errno = MPIR_Localcopy((void*)((char*)shmem_buffer), chunk_count_intra * ppn, datatype, 
										(void*)((char*)recvbuf), chunk_count_intra * ppn, datatype);
			}else{
				//NonBlocked
                if (SHOW_FINAL_RESULT && count==8192) fprintf(stderr,"[%d] is_global_block is 0\n",rank_index);
				int s=0;
				for(; s<ppn; ++s){
					
					mpi_errno = MPIR_Localcopy((void*)((char*)shmem_buffer + s * chunk_count_intra  * extent), chunk_count_intra , datatype, 
											(void*)((char*)recvbuf + ((comm_ptr->dev.ch.rank_list[s])- (node_id * ppn)) * chunk_count_intra * extent), chunk_count_intra, datatype);
					
				}
			}
#endif		
 if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[10] Unsec-ML-Ring [r=%d]\n",rank);	
			if (mpi_errno) {
				MPIR_ERR_POP(mpi_errno);
			}
	...
	if (PRINT_Ring_FUN_NAME && Print_Ring)     fprintf(stderr,"[11] Unsec-ML-Ring [r=%d]\n",rank);    
#if ALLREDUCE_PRINT_FUN  
    if (SHOW_FINAL_RESULT && count==8192) {
        int s , match=1;
		step = (count/8)*extent;  
...
if (0 && SHOW_FINAL_RESULT && count==8192 ) {

            void * print_chunk = recvbuf; 

            int tempp = step;
            step = extent;

            fprintf(stderr,"[End of Rank = %d]  print_chunk = %d  %d  %d  %d  %d  %d  %d  %d %d  %d  %d  %d count=%d   host=%s  [end]\n",rank,*((int *) (print_chunk)) , *((int *) (print_chunk+step)),*((int *) (print_chunk+2*step)),*((int *) (print_chunk+3*step)),*((int *) (print_chunk+4*step)),*((int *) (print_chunk+5*step)),*((int *) (print_chunk+6*step)),*((int *) (print_chunk+7*step)),*((int *) (print_chunk+8*step)),*((int *) (print_chunk+9*step)),*((int *) (print_chunk+10*step)),*((int *) (print_chunk+11*step)),count,hostname1);
...
#if 0    
    if (SHOW_FINAL_RESULT && count==8192 && 0) {
        int s , match=1;
		step = (count/8)*extent;  
        for (s=1 ; s<count ; s++){
            if ( (*((int *) (recvbuf))) != ( *((int *) (recvbuf+s*extent)))){
                fprintf(stderr,COLOR_RED"[%d] dismatch at %d th element (%d , %d , %d) count = %d   host = %s  [end]"COLOR_RESET"\n",
                rank,s, *((int *) (recvbuf+extent)),*((int *) (recvbuf+(s-1)*extent)),*((int *) (recvbuf+s*extent)),count,hostname1);
                match =0;
                break;
            }
        ...
        if (match==1) {
            //fprintf(stderr,COLOR_GREEN"[%d] is match host=%s [end]"COLOR_RESET"\n",rank,hostname1);
			fprintf(stderr,"[pt2pt_ring Rank = %d] is match [value = %d] host=%s  count=%d  [end]"COLOR_RESET"\n",comm_ptr->rank,*((int *) (recvbuf+extent)),hostname1,count);
        } else       
            fprintf(stderr,COLOR_CYAN"[pt2pt_ring Rank = %d]  recvbuf = %d  %d  %d  %d  %d  %d  %d  %d %d  %d  %d  %d count=%d   host=%s  [end]"COLOR_RESET"\n",rank,*((int *) (recvbuf)) , *((int *) (recvbuf+step)),*((int *) (recvbuf+2*step)),*((int *) (recvbuf+3*step)),*((int *) (recvbuf+4*step)),*((int *) (recvbuf+5*step)),*((int *) (recvbuf+6*step)),*((int *) (recvbuf+7*step)),count,hostname1);
    }
#endif	

    /*
    if (rank==31 && count==8192) {
        FILE *f = fopen("11.data", "wb");
        fwrite((float *) recvbuf, sizeof(MPI_FLOAT), count, f);
        fclose(f);
    }
    */
    ...
    if (PRINT_Ring_FUN_NAME)     fprintf(stderr,"[12] Unsec-ML-Ring [r=%d]\n",rank);
